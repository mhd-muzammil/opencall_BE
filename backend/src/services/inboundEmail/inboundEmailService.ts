    ...
    try {
      ...
      if (list.length > 0) {
        for await (const message of client.fetch(
          ...
          if (message.headers) {
            for (const line of message.headers.toString().split(/\r?\n/)) {
          ...
          const rawBody = message.bodyParts?.get("text")?.toString() ?? "";
          const preview = toPreview(rawBody);
          const fullText = toFullText(rawBody);
